    ...
    <div class="col-lg-9">
        ...
        <c:if test="${not empty users}">
            ...
            <c:forEach var="user" items="${users}">
                <c:if test="${user.status.name ne 'В ожидании'}">
                    <div class="card my-3">
                        <div class="card-body">
                            <c:if test="${user.status.name eq 'Активный'}">
                                ...
                                <c:if test="${'Администратор' eq sessionScope.role}">
                                    <a href="<c:url value="/controller?command=deactivate-account&userId=${user.userId}"/>"
                                       class="btn btn-outline-danger"><fmt:message key="users.deactivate"/></a>
                                </c:if>
                            </c:if>
                            <c:if test="${user.status.name eq 'Неактивный'}">
                                <h5 class="card-title text-secondary">${user.name} ${user.surname}
                                    (${user.username})</h5>
